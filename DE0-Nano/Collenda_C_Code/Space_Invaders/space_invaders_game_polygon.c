...
int main(){
	...
	spaceship.coord_x = 310; spaceship.coord_y = 300;
	spaceship.step_x  = 5; 	 spaceship.step_y  = 5; spaceship.direction = LEFT; spaceship.ativo = 1; spaceship.collision = 0; //spaceship.offset = 0;
	...
	int form        = 1;
	int opcode      = 0b0011;
	int color       = 0b111010101;
	unsigned int dataA = address;
	dataA = dataA << 4;
	dataA = dataA | opcode;
	unsigned int dataB = built_instruction(address, opcode, color, form, mult, spaceship.coord_x, spaceship.coord_y);
		...
		while(spaceship.collision == 0){
			...
			if(IORD(SCREEN_BASE,0) == 1){ // Checks if a screen has finished drawing.
				...
				for(int i = 0; i < number_of_aliens; i++) {
					...
					if((*enemy).ativo == 1){
						// Jogando o ponto de referência do polígono para o ponto inicial do quadrado que o circunscreve.
						spaceship.coord_x = spaceship.coord_x - 10;
						spaceship.coord_y = spaceship.coord_y - 10;
			...
			if(screen_ship == 2){
				...
				if(state_game == 0) {
					...
					if(refresh_ship){
						...
						//set_sprite(spaceship.data_register, spaceship.coord_x, spaceship.coord_y, spaceship.offset, spaceship.ativo);
						dataB = built_instruction(address, opcode, color, form, mult, spaceship.coord_x, spaceship.coord_y);
						sendInstructionToProcessor(dataA, dataB);
					}
